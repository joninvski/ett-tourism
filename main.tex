...
\runningheads{P. C. Sousa \emph{et al}}{CitySDK Tourism API - Building value around open data}

\articletype{Special Issue - Smart Cities}

\title{CitySDK Tourism API - Building value around open data}
\author{Pedro Cruz Sousa\affil{1},
...
In an effort to promote their attractions to tourists, some cities have adopted the open-data model, publishing data for programmers to use in their own applications.
Unfortunately, each city publishes touristic information using its own standard.
...
This would help developers in making cross-city applications, lowering the overhead of supporting new cities and providing cities with an increased exposure. Finally, tourists will also benefit from better and cheaper applications due to the boosted competition.

This article presents the CitySDK Tourism API, a library which aims to provide standardized access to information about Points of Interest, Events and Itineraries.
...
However, often the bases required for tourism applications continues to be accurate, high quality information provided by authoritative sources.
...
In a first approach, municipalities created applications (web or mobile) for sharing data with the tourists.
This option is costly and unsuitable for most cities: the number of visitors is not large enough to recuperate the investment; municipalities are not software houses and are unable to keep up with the pace of innovation. 
...
Municipalities may still be forced to publish some applications on their own, should they deem necessary, if the market does not believe them to be lucrative. 
...
Although events in EventsML-G2 can convey information about a \ac{poi} where an event takes place, that is not its focus.
...
The data model is shown in Figure~\ref{fig:data-model}~\cite{w3c-poi}. % TODO - I would put the cite command in the caption of the image. CHECK IT 
\begin{figure*}
\centering
...
The \textbf{Events} are modelled the same way the \emph{\acp{poi}} were, but instead of having a \textit{Location} entity completely specified, we used the \textit{relationship} property of the same entity to relate a given Event to a \emph{\ac{poi}} and omitted the \textit{address} and \textit{undetermined} properties. So, we have an \emph{Event} completely described using the \textit{POI} entity and use the \textit{relationship} property to also specify and describe the location of the \emph{Event}. An \emph{Events} list is modeled using the \textit{POIS} entity, much like the \emph{\acp{poi}}, but it does not have a different granularity and the root name is \textit{event} instead of \textit{poi}.

The \textbf{Itineraries} data model is somewhat more complex. It is defined by using the \textit{POIS} entity and all of its descriptive properties. So, we have the description of the itinerary itself by using the \textit{POIType} descriptive properties and have the group of \emph{\acp{poi}} by using the \emph{\textit{poi}} property named as \textit{pois} (so not to confuse with the mentioned list of \emph{\ac{poi}}). It should be mentioned that these \emph{\acp{poi}} are not the original \emph{\acp{poi}}, but are described in the context of the \emph{Itinerary}, though they include the relationship with its original counterpart, so to fetch the actual description. Finally and like the previous two, the \emph{Itineraries} has a list associated with it. Much like the \ac{poi}, it has a second granularity - a minimal version - in which only the description of each \emph{Itinerary} is included and their \emph{\acp{poi}} are omitted.

The \textbf{Categories/Tags} are equal in nature, but a \emph{Category} provides a recursive format that the \emph{Tags} do not. Both borrow from the W3C POI Data Model, but their format is more specific to the needs of the API, rather than following the mentioned model. So, a \emph{Category} simply follows the \textit{POIType} entity and allows recursiveness and a \emph{Tag} borrows its properties from the \textit{POIBaseType} to specify a language and value.
...
Figure~\ref{fig:architecture} shows the composing architectural components of the implemented CitySDK platform.
...
%The CitySDK project established a performance requirement in terms of minimum number of requests per second that the platform should be able to serve, calculated from the indicators in terms of expected public adherence to the CitySDK platform at the end of the project timespan through the applications to be developed by 3rd party developers. 
%The calculated number was 105 requests per second. 
%In order to assure that the performance requirement of the CitySDK was fulfilled, a simple test using 150 distinct requests for \acp{POIs} were performed against a CitySDK platform loaded with a database populated with real data from Lisbon municipality containing around 3800 independent \acp{poi}. 
%The tool used to perform the test was Siege\footnote{\url{http://www.joedog.org/siege-home/}} version 2.70 and the testing server was running in a Microsoft Windows 7 64bit powered by an Intel i5 processor at 2.9GHz with 8GB of RAM memory.
%A batch of 5 tests composed by 10.000 repetitions of 150 independent \acp{POIs} requests was performed resulting on the final average capacity of 620 requests per second which proved that the implemented CitySDK platform was fully compliant with the required performance capacity.
...
At last, we developed an Augmented Reality application using the Layar framework\footnote{http://www.layar.com/} by developing a PHP web-server, thus using the PHP library. 
...
The Objective-C library was not ignored, since an application is being developed at the moment. % TODO - I don't like the sentence
...
Tourist will also benefit from the broad availability of the applications and the quality of the data.
...
The same can be said about venue owners, which may wish to provide their own CitySDK Tourism API endpoints, were detailed information about the venue (i.e.\ \ac{poi}) and events may be provided.
