...
\item \textbf{POIType} is an abstract entity derived from POIBaseType and adds entities for describing, labeling, categorizing and indicating the time span of a \ac{POI} or group of \ac{POIs}.
This entity also incudes linking elements to other POIs, external web resources or metadata;
\item \textbf{Location} is an entity the inherits from POIBaseType and provides a flexible description of the location of a \ac{POI}.
...
This model is flexible enough to be used within our API to model the various types of data (\ac{poi}, events and thematic itineraries) required.
It use is described in section~\ref{section:w3cpoi}.
...
This standard is a member of the family of the IPTC G2-Standards, built on a structural and function framework called the IPTC News Architecture (NAR), and shares many of its components with the other standards of this family. 
